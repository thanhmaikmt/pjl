...
"""Dr.Leonards code, but the fitness function, controller, and reaping where modified by me"""


RUN_NAME="hover"             # used for file names so you can tag different experiments


# The world
WORLD_FILE="Y_hover.txt"     # world to use
...
VELOCITY_SCALE=1.0/30      # scale velocity (pod starts to slip at 80)
MAX_AGE=50                # pods life span   
REPROVE_PROB=0.3            # probability that selection we trigger a reprove of the best gene
N_HIDDEN=3                 # number of neurons in hidden layer
...
# print str1
        self.fontMgr.Draw(screen, None, 20,str1,(20,20), (0,255,0) )
        
        self.fontMgr.Draw(screen, None, 20,"x",(XREF,yHover), (255,255,255) )
...
class GAControl:
    ...
    def calc_fitness(self,state,brain):
        ...
        dist_TARGET=sqrt((state.y-yHover)**2)
         
            
        if pod.age >= MAX_AGE:
            fit= -dist_TARGET - poderror_integral/MAX_AGE
            print "Fit of age: " + str(fit) 
            return fit
        if pod.collide:
            #print "collision"
            fit = - 100000
            print "fit of col :" + str(fit)
            return fit
    ...
    def process(self,sensor,state,dt):
        ...
        dist_TARGET=sqrt((state.y-yHover)**2)
        try:    
            poderror_integral += dist_TARGET*dt
