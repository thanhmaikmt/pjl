...
class DispersionLayout1 implements LayoutManager {

    public DispersionLayout1() {
    }
...
class DispersionFrame extends Frame
    ...
    public void init() {
        ...
        main.setLayout(new DispersionLayout1());
    ...
    public void updateDispersion(Graphics realg) {
        ...
        double tadd = 0;
        double f[] = {freq1Bar.getValue() * .1, freq2Bar.getValue() * .1};
        double s[] = {speed1Bar.getValue() * f[0] / 30., speed2Bar.getValue() * f[1] / 30.};
        ...
        for (int k = 0; k < view.length; k++) {
            if (view[k] != null) {
                double mult = 2 * pi * f[k] / sampleCount;
                for (i = 0; i != sampleCount; i++) {
                    func[i] = (t[k] < mult * i) ? 0 : .5 * Math.sin(t[k] - mult * i);
                }
                drawFunction(g, view[k], func, null, sampleCount, 0);
            }
        }
        ...
        if (viewSum != null) {
            for (i = 0; i != sampleCount; i++) {
                ...
                func[i] = 0;
                for (int ii = 0; ii < t.length; ii++) {
                    double mult = 2 * pi * f[ii] / sampleCount;

                    if (t[ii] >= mult * i) {
                        func[i] += 0.5*Math.sin(t[ii] - mult * i);
                    }
                }
            }
