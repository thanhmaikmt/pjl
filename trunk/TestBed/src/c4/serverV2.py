...
# using "" for host name seems to work best. Explicitly naming the host seemed to confuse matters
port=4004
host=""
...
print "TCPServer Waiting for client on"+host+":"+str(port)
            ...
            if len(data) == 0  or data[0] == 'Q' or data[0] == 'q':   
                ...
                for key,c in connections.items():
                    if c.game == None:
                        ...
                        self.send("+"+key+"_"+c.game.opo_of(key))
                ...
                if self.myopo() != None:
                    ...
                    if opo == None:
                        ...
                        self.send("? sorry you can not play against your self ")
                    else:
                        if opo.game == None:
                            self.game=Game(self,opo,data[0])
                            opo.game=self.game;
                            self.send("! "+ opo_name + " accepted")      # important to send acceptance before invite    
                            self.myopo().send(data[0]+" "+self.name)     # (otherwise we can get a response from opo before acknowledgement)
                            
                        else:
                            self.send("? "+ opo.name+ " is already playing "+opo.myopo().name)
