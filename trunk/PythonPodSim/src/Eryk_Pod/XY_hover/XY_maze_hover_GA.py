...
print "1 - Pool only trained to hover, but not for sensors"
print "2 - Poolt rained to get to the X"
print "3 - Run as normal"
...
class GAControl:
    ...
    def calc_fitness(self,state,brain):
        ...
        fitness= pod.age-100000 + (state.pod.pos_trips-state.pod.neg_trips)*10
        
        if (state.pod.pos_trips-state.pod.neg_trips) == 11:
                #print "3re colision"
                
                fitness = 1500 -dist_FROM_REF2
                print fitness
...
dt          =.1       
sensorRange = 1000
pool=Pool(5+N_SENSORS,N_HIDDEN,4)
pods=[]
