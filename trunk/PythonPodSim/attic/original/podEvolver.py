...
#  A good_list (Pool) of the best pods is kept.
...
class Pool:  #  use me to store the best brains and create new brains
    ...
    def add_fluke(self,gene):  
        ...
        for i in range(len(self.fluke_list)):
            if gene.fitness > self.fluke_list[i].fitness:
                self.fluke_list.insert(i,gene)
                if len(self.fluke_list) > self.maxFlukeMembers:
                    self.fluke_list.pop()
                return
            
        if len(self.fluke_list) < self.maxFlukeMembers:
            self.fluke_list.append(gene)    
    ...
    def add(self,gene):  
        ...
        if len(self.good_list) >= self.maxMembers:
            if gene.fitness < self.good_list[self.maxMembers-1].fitness:
                return
          
        for i in range(len(self.good_list)):
            if gene.fitness > self.good_list[i].fitness:
                self.touched=True
                self.good_list.insert(i,gene)
                if len(self.good_list) > self.maxMembers:
                    self.good_list.pop()
                return
            
        if len(self.good_list) < self.maxMembers:
            self.good_list.append(gene)    
            self.touched=True
    ...
    def create_best(self):
        clone=self.good_list[0].brain.clone()
    ...
    def select2(self):
        ...
        if len(self.fluke_list) > 0 and  random() < CHOOSE_FLUKE_PROB:
            ...
            return self.fluke_list[id].brain.clone()
        ...
        return self.good_list[id].brain.clone()
